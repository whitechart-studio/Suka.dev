...
When `agents detect --publish` or `agents watch` publishes presence, Suka marks it with `presence.source.kind: detected`. The source metadata includes the required detector name and may also include process ID, repository cwd, and detection timestamp so dashboards and CLI summaries can distinguish detected presence from manually published presence.
