...
6. Keep platform compatibility explicit: full developer tooling must work on Windows, Linux, and macOS; iOS must be supported as a dashboard/client target.
...
- preserves cross-platform behavior across path handling, shell commands, filesystem watching, networking, and browser access
...
- assume Unix-only shell behavior, POSIX paths, Linux-only filesystem watchers, or desktop-only dashboard layouts
